...
public class MusicWaterActivity extends ListActivity {
	public List<String> mMusicList=new ArrayList<String>();//?????งา??????
	private static final String MUSIC_PATH=new String("/sdcard/Water");//????????กค??
	public MediaPlayer mMediaPlayer=new MediaPlayer();//Mediaplayer???????????????
	private int currentListItme=0;//???????ID
	private ImageView f_musicwater;
	ListView lView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_music_water);
	    ListView lView= (ListView) findViewById(android.R.id.list);
	    f_musicwater=(ImageView)findViewById(R.id.fanhui_water);
...
//					String musicname3="Robert Haigh1_1.1";
				    ...
				    Intent intent=new Intent(getApplicationContext(),MusicWaterPlayActivity.class);
				    intent.putExtra("Water", musicname);
				    intent.putExtra("PositionW", position);
	    ...
	    //???????
	    f_musicwater.setOnClickListener(new OnClickListener() {
		...
		public void musicList() {
			...
			if (home.listFiles(new MusicFilter()).length >0) {
				...
				ArrayAdapter<String> musicList = new ArrayAdapter<String>(
					  MusicWaterActivity.this, android.R.layout.simple_list_item_1, mMusicList);
